...
Kotlin 公式スタイル（インデント 4 スペース）。`@Composable` は PascalCase、一般関数は camelCase。パッケージは `org.example.project`、Android リソースは `snake_case`。テーマ変更は `AppTheme.kt`/`Color.kt` を更新し、エントリポイントでは `App()` を直接呼び出してください（`App()` 内部で `AppTheme` を適用しています）。
